# Delta for Dashboard Operator UI
...
### Requirement: Clear Information Architecture and Navigation
The dashboard MUST organize the operator experience into clear top-level areas for Overview, Accounts, Configuration, and Operations, and it MUST expose Antigravity only when the corresponding feature is enabled. Navigation MUST make it obvious where operators can inspect health, manage accounts, adjust configuration, and run operational actions.
...
- THEN the UI MUST present clear navigation or sectioning for Overview, Accounts, Configuration, and Operations
- AND the Antigravity area MUST appear only when the feature is enabled instead of occupying permanent space when unavailable
